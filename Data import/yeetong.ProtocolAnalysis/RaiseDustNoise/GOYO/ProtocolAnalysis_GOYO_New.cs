...
{
    public class ProtocolAnalysis_New
    {
        public static string OnResolveRecvMessage(byte[] b, int c, TcpSocketClient client)
        {
            try
            {
                ...
                if (typ == 0x03)
                {
                    try
                    {
                        ToolAPI.XMLOperation.WriteLogXmlNoTail("OnResolveRecvMessage-->收到参数配置应答", "应答内容：" + Encoding.ASCII.GetString(b));
        ...
        public static byte[] OnResolveParm(byte[] b, int bCount, TcpSocketClient client, ref DBFrame df)
        {
            ...
            try
            {
                PM2_5_Factor = Convert.ToDouble(b[35].ToString());
                PM10_Factor = Convert.ToDouble(b[36].ToString());
                TSP_Factor = Convert.ToDouble(b[37].ToString());
            }
            catch
            {
            ...
            ToolAPI.XMLOperation.WriteLogXmlNoTail("扬尘参数上传", df.contentjson);
            if (df.contentjson != null && df.contentjson != "")
            {
                DB_MysqlRaiseDustNoise.SaveRaiseDustNoise(df);
            }
